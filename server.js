  ...
  if (alreadyValidated) {
    if (explicitGroupAccessRequest) {
      return buildResponse({
        ...
        has_control_update: "ja",
        interested_in_control_update: "ja",
      });
    }
  ...
  if (orderControlResponse) {
    ...
    if (isValidationEvent) {
      const extractorMessage =
        ...
        "De klant heeft Control besteld. Werk last_summary bij om deze nieuwe state te weerspiegelen.]";
      ...
      try {
        ...
        if (extractionResult.status === "fulfilled") {
          ...
          // Merge extractor output, but keep the order-control-set fields
          // (customer_status, current_phase, has_control, interested_in_control)
          // since those are authoritative after a validated order.
